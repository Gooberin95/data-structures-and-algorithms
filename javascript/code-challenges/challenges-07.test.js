...
const howMuchPencil = (str) => {
  ...
  const result = [str]; // add original string to the result array

  for (let i = 1; i <= str.length; i++) {
    const newStr = str.slice(i); // remove the first i characters
    result.push(newStr); // add the new string to the result array
  }
